...
def discover_new_nations(session: requests.Session) -> List[str]:
    ...
    try:
        # API endpoint for daily dump of all nations
        # We'll query the last 20 founded nations
        params = {"q": "nations", "shard": "lastfounded"}
        ...
        nations = []
        for nation_elem in root.findall(".//nation"):
            nation_name = nation_elem.text
            if nation_name:
                nations.append(nation_name.strip())
...
def main_loop() -> None:
    ...
    logging.info("Autopilot started. Loaded %d previously sent nations. Press Ctrl+C to stop.", len(sent_nations))
